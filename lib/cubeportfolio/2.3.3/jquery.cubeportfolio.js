...
(function($, window, document, undefined) {
    ...
    var popup = {
        ...
        openLightbox: function(blocks, currentBlock) {
            ...
            $.each(blocks, function(index, item) {
                ...
                if ($.inArray(href, tempHref) === -1) {
                    ...
                    //添加流播放器
                    else if (/\.flv$/i.test(href)||/rtmp:/i.test(href)) {
                        src = href;
                        type = 'isFlowplayer';
                    }
         ...
         * @param el
         */
        isFlowplayer: function(el) {
            var t = this;
            t.updateFlowplayer(el.src, el.title, t._getCounterMarkup(t.options.lightboxCounter, t.current + 1, t.counterTotal));
        },
         ...
         * 添加流的播放器
         */
        updateFlowplayer: function(src, title, counter) {
            var t = this;
            t.wrap.addClass('cbp-popup-lightbox-isIframe');
            var markup = '<div class="cbp-popup-lightbox-figure" style="height:600px">' +
                    '<a href="'+src+'"/>'
                //'<iframe src="' + src + '" frameborder="0" allowfullscreen scrolling="no"></iframe>' +
                '<div class="cbp-popup-lightbox-bottom">' +
                ((title) ? '<div class="cbp-popup-lightbox-title">' + title + '</div>' : '') +
                ...
                '</div>';
            t.content.html(markup);
            t.content.css("width","350px");
            var find = t.content.find("a");
            t.createFlowplayer(find,$cons.playerMode.RTMP);
            t.wrap.addClass('cbp-popup-ready');
            //t.preloadNearbyImages();
        },
        /**
         * 穿件流播放器
         */
        createFlowplayer:function(el,plaerMode){
            var flvPath = "/"+$route.getJs("flowplayer","flowplayer-3.2.18.swf");
            var rtmpPath = "/"+$route.getJs("flowplayer","flowplayer.rtmp-3.2.13.swf");
            //var that = t;
            var url = el.attr("href");
            if(plaerMode==$cons.playerMode.FLV){
                el.flowplayer( flvPath,{

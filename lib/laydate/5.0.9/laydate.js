      ...
      getPath: function(){
       var jsPath = document.currentScript ? document.currentScript.src : function(){
        var js = document.scripts
            ,last = js.length - 1
            ,src;
        var jsStr = "laydate.js";
        for(var i = last; i > 0; i--){
         //1.修改，改成以获取laydate.js为后缀的script标签
            //处理掉？后面的数据值
            var indexOf = js[i].src.indexOf(jsStr);
            if(indexOf>0){
                src = js[i].src.substring(0,indexOf);
                break;
            }
            /*var jsPath = js[i].src.substring(0,js[i].src.indexOf("?"));
            if(jsPath.slice(-jsStr.length) == jsStr){
                src = jsPath;
                break;
            }*/
